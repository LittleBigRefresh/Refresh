...
public partial class GameAsset
{
    ...
    public string? GetAsPhoto(TokenGame game, IGameDatabaseContext database, IDataStore dataStore)
    {
    ...
    /// <param name="dataStore">The data store</param>
    /// <returns>The new hash of the converted asset, or null if no conversion has taken place</returns>
    public string? GetAsIcon(TokenGame game, IGameDatabaseContext database, IDataStore dataStore)
    {
        return this.GetAsGeneric(
            game,
            database,
    ...
    private string? GetAsGeneric(TokenGame game, IGameDatabaseContext database, IDataStore dataStore, Func<Image, Image<Rgba32>?> transformImage, Func<string?> getMainline, Action<string> setMainline,
