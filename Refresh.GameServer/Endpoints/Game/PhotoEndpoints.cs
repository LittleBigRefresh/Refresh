...
public class PhotoEndpoints : EndpointGroup
{
    ...
    private static Response GetPhotos(RequestContext context, IGameDatabaseContext database, IDataStore dataStore, TokenGame game, Func<GameUser, int, int, DatabaseList<GamePhoto>> photoGetter)
    {
    ...
    public Response PhotosWithUser(RequestContext context, IGameDatabaseContext database, IDataStore dataStore, Token token) 
        => GetPhotos(context, database, dataStore, token.TokenGame, database.GetPhotosWithUser);
    
    [GameEndpoint("photos/by", ContentType.Xml)]
    [Authentication(false)]
    [MinimumRole(GameUserRole.Restricted)]
    public Response PhotosByUser(RequestContext context, IGameDatabaseContext database, IDataStore dataStore, Token token) 
