...
public class ApiLevelCategoryResponse : IApiResponse, IDataConvertableFrom<ApiLevelCategoryResponse, LevelCategory>
{
    ...
    public static ApiLevelCategoryResponse? FromOldWithExtraData(LevelCategory? old, IGameDatabaseContext database, IDataStore dataStore, GameLevel? previewLevel)
    {
        if (old == null) return null;
    ...
    public static IEnumerable<ApiLevelCategoryResponse> FromOldListWithExtraData(IEnumerable<LevelCategory> oldList, IGameDatabaseContext database, IDataStore dataStore) 
